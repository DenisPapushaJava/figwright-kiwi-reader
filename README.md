...
- обслуживать несколько MCP-клиентов через один локальный HTTP-хаб;
...
| `use_file`            | Выбрать активный файл в отдельном stdio-процессе    |
...
| `save_assets`         | Сохранить доступные SVG и растровые ресурсы         |
...
node "$env:LOCALAPPDATA\FigwrightKiwi\server\hub.mjs"
```

В каждом клиенте укажите Streamable HTTP URL `http://127.0.0.1:9225/mcp` и заголовок
`Authorization: Bearer <тот же секрет>`. Хаб один раз занимает порт захвата 9224 и хранит общий
scenegraph в памяти. При нескольких вкладках агент вызывает `list_files` и передаёт `fileKey` или
`tabId` в конкретное чтение; общего переключателя `use_file` в HTTP-режиме нет.
...
6. Если для вёрстки нужны фотографии и другие растровые заливки, включите
   **Захватывать растровые изображения**. По умолчанию опция выключена; изменение применяется к
   следующему подключению или повторному чтению.
7. Нажмите **Подключить макет**. Расширение подключится к вкладке и один раз перезагрузит её.
8. Дождитесь зелёного состояния готовности и выполните запрос в Codex.

После обновления файлов распакованного расширения обязательно нажмите **Обновить** на его карточке
в `chrome://extensions` и заново откройте popup или боковую панель. Chrome не перезапускает уже
работающий service worker при обычном изменении файлов на диске. Новый интерфейс распознаёт старый
worker, отключает несовместимый checkbox и показывает код `EXTENSION_RELOAD_REQUIRED`.
...
подключённых файлов, агент использует `list_files`: в stdio-режиме — затем `use_file`, в общем
HTTP-хабе — передаёт `fileKey` или `tabId` в каждый запрос.
...
| `EXTENSION_RELOAD_REQUIRED` | Нажмите **Обновить** для расширения в `chrome://extensions` и откройте панель заново |
