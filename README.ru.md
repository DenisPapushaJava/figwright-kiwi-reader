...
Код проекта при этом не исполняется. Импортированный или сложный тип остаётся помечен как неполный,
поэтому Reader не выдаёт отсутствующие props за доказанный дефект UI-kit. Для Vue, Svelte и Angular
переносимый сканер пока сопоставляет только имена компонентов.
