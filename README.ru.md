...
@fk прочитай выбранный фрейм и сопоставь компоненты, иконки и токены с текущим проектом
```
...
При реализации в существующем проекте агент вызывает `component_map`, `icon_map` и `token_map` с
корнем этого проекта. `token_map` читает реально использованные цвета заливок, обводок, градиентов,
теней и смешанного текста и ищет точное совпадение в CSS custom properties и SCSS variables.
